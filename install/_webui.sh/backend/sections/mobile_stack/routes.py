"""
Mobile Services Stack Section Routes
Handles mobile service configuration (WebRTC, mobile proxies, PWA backends)
"""
...
async def get_services():
    config = load_config()
    return config.get("services", {}).get("mobile", [])
...
async def add_service(service: Service):
    ...
    if "services" not in config:
        config["services"] = {}
    if "mobile" not in config["services"]:
        config["services"]["mobile"] = []
    config["services"]["mobile"].append(service.dict())
    save_config(config)
    return {"status": "added"}
...
async def update_service(service_id: str, update: ServiceUpdate):
    config = load_config()
    services = config.get("services", {}).get("mobile", [])
    ...
    config["services"]["mobile"] = services
    save_config(config)
...
async def update_all_services(services: List[Service]):
    ...
    if "services" not in config:
        config["services"] = {}
    config["services"]["mobile"] = [s.dict() for s in services]
    save_config(config)
    return {"status": "updated"}
