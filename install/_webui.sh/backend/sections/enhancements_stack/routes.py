"""
Enhancements Services Stack Section Routes
Handles enhancement service configuration (STT/TTS helpers, dashboards, etc.)
"""
...
async def get_services():
    config = load_config()
    return config.get("services", {}).get("enhancements", [])
...
async def add_service(service: Service):
    ...
    if "services" not in config:
        config["services"] = {}
    if "enhancements" not in config["services"]:
        config["services"]["enhancements"] = []
    config["services"]["enhancements"].append(service.dict())
    save_config(config)
    return {"status": "added"}
...
async def update_service(service_id: str, update: ServiceUpdate):
    config = load_config()
    services = config.get("services", {}).get("enhancements", [])
    ...
    config["services"]["enhancements"] = services
    save_config(config)
...
async def update_all_services(services: List[Service]):
    ...
    if "services" not in config:
        config["services"] = {}
    config["services"]["enhancements"] = [s.dict() for s in services]
    save_config(config)
    return {"status": "updated"}
